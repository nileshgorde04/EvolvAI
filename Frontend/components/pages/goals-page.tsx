...
const api = {
  get: async (endpoint: string, token: string) => { const res = await fetch(`http://localhost:8080/api${endpoint}`, { headers: { 'Authorization': `Bearer ${token}` } }); if (!res.ok) throw new Error("Failed to fetch"); return res.json(); },
  post: async (endpoint: string, body: any, token: string) => { const res = await fetch(`http://localhost:8080/api${endpoint}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(body) }); if (!res.ok) { const err = await res.json(); throw new Error(err.message); } return res.json(); },
  put: async (endpoint: string, body: any, token: string) => { const res = await fetch(`http://localhost:8080/api${endpoint}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(body) }); if (!res.ok) throw new Error("Failed to update"); return res.json(); },
  delete: async (endpoint: string, token: string) => { const res = await fetch(`http://localhost:8080/api${endpoint}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } }); if (!res.ok) throw new Error("Failed to delete"); return res.json(); }
}
...
export function GoalsPage() {
  ...
  useEffect(() => {
    const fetchGoals = async () => {
      ...
      try {
        ...
        setGoals(userGoals);
      } catch (error) { toast.error("Error fetching goals."); } 
  ...
  const handleAIGenerate = async () => {
      ...
      const token = localStorage.getItem('token');
      if (!token) { setIsAIGenerating(false); return; }
      try {
  ...
  const handleToggleTask = async (taskId: string, isCompleted: boolean) => {
      ...
      try {
          ...
          setGoals(goals.map(g => g.id === activeGoal.id ? { ...g, tasks: g.tasks.map(t => t.id === taskId ? { ...t, is_completed: !isCompleted } : t) } : g));
      } catch (error) { toast.error("Failed to update task."); }
  };

  const calculateProgress = (tasks: Task[] = []) => tasks.length === 0 ? 0 : Math.round((tasks.filter(t => t.is_completed).length / tasks.length) * 100);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2"><Target className="h-6 w-6 text-purple-400" /><h1 className="text-3xl font-bold text-white">My Main Goal</h1></div>
        <Dialog open={isAddGoalOpen} onOpenChange={setIsAddGoalOpen}>
          <DialogTrigger asChild><Button className="bg-purple-600 hover:bg-purple-700"><Plus className="mr-2 h-4 w-4" /> New Goal</Button></DialogTrigger>
          <DialogContent className="glass-card border-white/10">
            <DialogHeader><DialogTitle className="text-white">Create a New Goal</DialogTitle></DialogHeader>
            <div className="space-y-4">
              <Input placeholder="E.g., Learn Next.js" value={newGoalTitle} onChange={(e) => setNewGoalTitle(e.target.value)} className="bg-transparent" />
      ...
      {isLoading && <p className="text-gray-400 text-center">Loading your goals...</p>}
        ...
        <>
          <Card className="glass-card border-white/10 bg-gradient-to-r from-purple-900/20 to-blue-900/20">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between"><span className="text-2xl font-bold text-white">{completionPercentage}% Complete</span><Sparkles className="h-6 w-6 text-yellow-400" /></div>
          ...
          <Card className="glass-card border-white/10">
            ...
            <CardContent>
              <div className="space-y-3">
                <AnimatePresence>
                  {activeGoal.tasks.map((task) => (
                    <motion.div key={task.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className={`flex items-center space-x-3 p-2 rounded-lg hover:bg-white/5 ${task.is_completed ? "bg-green-900/20 border-green-500/30" : "bg-white/5 border-white/10"}`}>
                      <Checkbox checked={task.is_completed} onCheckedChange={() => handleToggleTask(task.id, task.is_completed)} className="data-[state=checked]:bg-green-600" />
              ...
              <div className="flex space-x-2 mt-4">
                  <Input placeholder="Add a new task..." value={newTaskName} onChange={(e) => setNewTaskName(e.target.value)} className="bg-transparent" onKeyPress={e => e.key === 'Enter' && handleAddTask()} />
